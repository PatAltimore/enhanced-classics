---
title: "The Time Machine"
author: "H.G. Wells"
year: 1895
chapter: 12
chapter_title: "In the Darkness"
slug: "chapter-12-in-the-darkness"
book_slug: "the-time-machine"
license: "public-domain"

summary:
  - point: "The narrator finds himself in the oppressive darkness of the Morlocks' underground domain."
    link: "https://en.wikipedia.org/wiki/The_Time_Machine"
    link_label: "The Time Machine"
  - point: "The air is damp and metallic, adding to the eerie atmosphere of the tunnels."
    link: "https://en.wikipedia.org/wiki/Atmospheric_conditions"
    link_label: "Atmospheric conditions"
  - point: "The narrator senses the presence of the Morlocks, who remain hidden but audible in the shadows."
    link: "https://en.wikipedia.org/wiki/Morlock"
    link_label: "Morlocks"
  - point: "The stolen Time Machine is the narrator's goal as he navigates the labyrinthine tunnels."
    link: "https://en.wikipedia.org/wiki/Time_machine"
    link_label: "Time machine"
  - point: "The narrator reflects on the stark contrast between the worlds of the Eloi and the Morlocks in the distant future."
    link: "https://en.wikipedia.org/wiki/Dystopia"
    link_label: "Dystopia"

enhancements:
  - id: "air-heavy-with-damp"
    trigger: "air heavy with damp"
    title: "Atmosphere in Underground Spaces"
    wikipedia_url: "https://en.wikipedia.org/wiki/Atmospheric_conditions"
    image_url: ""
    image_caption: ""
    content: "The description of the air being 'heavy with damp' evokes the claustrophobic and oppressive conditions often found in subterranean environments. In literature, such atmospheric details are used to heighten tension and immerse readers in the setting. Underground spaces, like caves or tunnels, often have high humidity due to limited ventilation and proximity to groundwater. This can create a sense of discomfort and unease, which aligns with the narrator's experience in the Morlocks' domain. The damp air also symbolizes decay and stagnation, reinforcing the dystopian themes of H.G. Wells' future world."
  - id: "time-machine"
    trigger: "Time Machine"
    title: "The Concept of Time Travel"
    wikipedia_url: "https://en.wikipedia.org/wiki/Time_machine"
    image_url: ""
    image_caption: "Title page of 'The Time Machine' by H.G. Wells, Wikimedia Commons"
    content: "The Time Machine is central to H.G. Wells' exploration of time travel, a concept that has fascinated humanity for centuries. In this novel, the machine serves as a vehicle for the narrator to witness the distant future and the evolution—or devolution—of humanity. Wells popularized the idea of a mechanical device enabling time travel, influencing countless works of science fiction. The Time Machine also acts as a metaphor for human curiosity and the consequences of technological advancement. Its theft by the Morlocks underscores the tension between progress and regression, a recurring theme in Wells' work."
  - id: "juxtaposition-of-worlds"
    trigger: "juxtaposition of worlds"
    title: "Contrasting Eloi and Morlocks"
    wikipedia_url: "https://en.wikipedia.org/wiki/Dystopia"
    image_url: ""
    image_caption: ""
    content: "The 'juxtaposition of worlds' refers to the stark contrast between the surface-dwelling Eloi and the subterranean Morlocks in Wells' future society. This dichotomy serves as a critique of class divisions and industrialization. The Eloi represent a passive, decadent upper class, while the Morlocks embody the oppressed working class, forced to toil in darkness. Wells uses this division to explore the consequences of unchecked social and economic inequality. The juxtaposition also highlights the fragility of civilization, suggesting that humanity's advancements may ultimately lead to its downfall. This theme resonates with broader dystopian literature, which often examines the interplay between societal structures and human nature."
---
The darkness was thick and palpable, like a veil drawn tightly across my senses. I stood in the cavernous depths of the Morlocks’ domain, the **air heavy with damp** and faintly metallic odors that clung to my skin and clothes. The faint hum of machinery echoed distantly, a strange and mechanical heartbeat that pulsed through the labyrinthine tunnels. My lantern flickered weakly, its flame a frail beacon against the oppressive gloom. Around me, shadows stirred and shifted, and I knew that I was not alone.

The Morlocks were near. I could feel them—hear them, even—though they kept themselves just out of sight. Their low, guttural sounds reverberated in the darkness, accompanied now and then by the faint patter of their bare feet against the stone. It was a disquieting symphony of presence, a constant reminder that I had trespassed into their world. Yet there was no turning back. Somewhere in this subterranean maze lay my **Time Machine**, stolen and hidden away by these troglodytic beings.

I pressed forward, gripping the handle of the lantern with white-knuckled intensity. The feeble light revealed little more than jagged walls and rusted pipes, the remnants of a bygone age. The tunnels seemed endless, sprawling in all directions like the roots of some ancient, malevolent tree. I could only hope that my path would lead me to the machine before the Morlocks made their move.

As I walked, I reflected on the strange **juxtaposition of worlds** I had encountered in this distant future. Abo
